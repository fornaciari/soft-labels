...
###################################################################################################
    ...
    @staticmethod
    def plotcrypto_mtl(trn, dev, tst, path_pdf):
        x_range = range(1, trn.shape[0]+1)
        plt.subplots(nrows=2, ncols=2, sharex=True, figsize=(10, 10))
        ax = plt.subplot(221)
        ax.plot(x_range, tst.regre_loss, label='tst regre loss')
        ax.plot(x_range, dev.regre_loss, label='dev regre loss')
        ax.plot(x_range, trn.regre_loss, label='trn regre loss')
        ax.legend()
        ax = plt.subplot(222)
        ax.plot(x_range, tst.class_loss, label='tst class loss')
        ax.plot(x_range, dev.class_loss, label='dev class loss')
        ax.plot(x_range, trn.class_loss, label='trn class loss')
        ax.legend()
        ax = plt.subplot(223)
        ax.plot(x_range, tst.acc, label='tst acc')
        ax.plot(x_range, dev.acc, label='dev acc')
